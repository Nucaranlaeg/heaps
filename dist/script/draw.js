...
function getNodeReferences() {
    if (!heapPanel) {
        ...
        document.querySelector("#heaps").onmousedown = startPan;
        document.onmouseup = endPan;
    }
}
function redraw() {
    getNodeReferences();
    if (!clearNode(heapPanel))
        return;
    const buildOptions = [];
    heaps.forEach(h => {
        if (!h.node) {
            h.node = resourceTemplate.cloneNode(true);
...
function drawTotalStats() {
    ...
    heapViewTasks.push(() => {
        const totalSand = heaps.reduce((a, c) => a + (c.heapType === 0 /* HEAP_TYPE.STANDARD */ ? c.sand : 0), 0);
    ...
    heapViewTasks.push(() => {
        const loss = heaps.reduce((a, c) => a + (c.heapType === 0 /* HEAP_TYPE.STANDARD */ ? c.sandOutflow("void") * (ADJACENT_HEAPS - c.adjacent.length) : 0), 0) / TIME_FACTOR;
